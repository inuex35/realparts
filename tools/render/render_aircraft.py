"""The airliner, turned once.

    blender -b -noaudio --factory-startup -P tools/render/render_aircraft.py \
        -- build/plane [frames]

One solid, so one object -- what is worth looking at here is the surface, not
the assembly: every curve on it comes from a loft through sections, and a loft
is only as good as the order of its sections.
"""
import math
import os
import sys
...
OUT = argv[0] if argv else os.path.join(REPO, "build", "plane")
FRAMES = int(argv[1]) if len(argv) > 1 else 1
TILT = float(os.environ.get("PLANE_TILT", "0.30"))
START = float(os.environ.get("PLANE_TURN", "218"))
...
session.op_open(os.path.join(REPO, "examples", "aircraft", "airliner.json"))
session.op_build()
# fine, because the whole point of this model is curvature: at 0.4 the lofted
# fuselage showed its stations as facets
tri = tessellate(session.body, deflection=0.12, angular=0.25)
...
mesh = bpy.data.meshes.new("aircraft")
...
plane = bpy.data.objects.new("aircraft", mesh)
...
for index in range(FRAMES):
    ...
    camera.location = centre + direction * diagonal * 1.45
