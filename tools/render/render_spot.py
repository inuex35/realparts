...
Nine parts from three documents, and the shape is the point: the rounded slab,
the hip housings standing out at the corners, and the blade legs folded back.
The pose is in the document, not here -- a robot is a machine at an angle.
"""
import math
import os
import sys
...
OUT = argv[0] if argv else os.path.join(REPO, "build", "spot")
FRAMES = int(argv[1]) if len(argv) > 1 else 1
TILT = float(os.environ.get("SPOT_TILT", "0.22"))
START = float(os.environ.get("SPOT_TURN", "236"))
...
session.op_open(os.path.join(REPO, "examples", "spot", "spot.json"))
session.op_build()
# coarser than the aeroplane needs: this machine is flats and cylinders, and
# the only curvature that matters is on the feet
tri = tessellate(session.body, deflection=0.4, angular=0.3)
